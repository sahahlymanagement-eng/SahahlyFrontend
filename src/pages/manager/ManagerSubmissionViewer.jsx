...
export default function ManagerSubmissionViewer({ scope = "manager" }) {
  ...
  useEffect(() => {
    ...
    // (a results-modal re-render right after mount — e.g. the auto-save-on-
    // open flow replacing `resultModal` — used to fire two concurrent blob
    // GETs for the same large file, producing net::ERR_FAILED on one or
    // both in Chrome), so an ignore-flag is enough here; there is no
    // duplicate network request left running to cancel.
    let ignore = false;
    setMarkSchemeLoading(true);
    setMarkSchemeError(null);
    fetchMarkSchemeFile(api, aid)
      .then((file) => {
        if (ignore) return;
        if (!file) {
  ...
  // over and disable Return All on a different assignment selected next.
  useEffect(() => {
...
const fetchSavedResults = useCallback(async () => {
  ...
  try {
    const map = await fetchSavedResultsLight(api, selectedAssignment._id);
    const synced = {};
...
const refreshStudents = async () => {
  ...
  try {
    ...
    let savedMap = {};
    try {
      savedMap = await fetchSavedResultsLight(api, selectedAssignment._id);
...
const syncPdfMirror = async () => {
  ...
  try {
    const res = await api.post("/submission-files/mirror-sync", {
      assignmentId: selectedAssignment._id,
    });
    const data = res.data || {};
    if (data.skipped) {
...
useEffect(() => {
  ...
  const handleMsUpload = async (file) => {
    ...
    try {
      ...
      invalidateMarkSchemeFile(selectedAssignment._id);
      if (msPreviewRef.current.assignmentId === selectedAssignment._id) {
        if (msPreviewRef.current.url) URL.revokeObjectURL(msPreviewRef.current.url);
  ...
  const runAiReview = async (student, rowCtx) => {
    ...
    try {
      const [studentFile, msFile] = await Promise.all([
        ...
        fetchMarkSchemeFile(api, selectedAssignment._id),
      ]);
      ...
      const pdfUrl = URL.createObjectURL(studentFile);
 ...
 // runs whenever selected assignment changes
...
// Resume polling is handled by checkForActiveJob when the assignment is selected.
  ...
  const resolveSavedResultForModal = useCallback(
    async (student, { batch, bulk, single, db }) => {
      ...
      if (!selectedAssignment?._id || !student?.submissionId) {
        return {
  ...
  const returnToStudent = async () => {
    ...
    try {
      ...
      if (blocked) {
        ...
        toast.error(message, { autoClose: false, toastId: `classroom-blocked-${selectedAssignment?._id}` });
      } else {
        toast.error(message);
      }
    } finally {
  ...
  const handleReturnAll = async () => {
    ...
    if (classroomAttachBlocked) {
      toast.error(
        ...
        { autoClose: false, toastId: `classroom-blocked-${selectedAssignment._id}` }
      );
      return;
    }
                                  ...
                                  <td>
                                    <div className="ma-avatar-cell">
                                      ...
                                      {(() => {
                                        ...
                                        const totalMismatchText =
                                          listMeta?.totalMismatchMessage ||
                                          totalMarksMismatchInfo(db?.result)?.message;
                                        const fileWarnText =
                                        ...
                                        if (!fileWarnText && !pageFlagText && !orientationFlagText && !totalMismatchText) return null;
                                  ...
                                  <td>
                                    {s.submissionId ? (
                                      <div className="msv-actions">
                                        ...
                                        {showMarkingTools && savedRowHasMarkingResult(db) && (
                                          <button
