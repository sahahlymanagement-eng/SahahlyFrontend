...
export default function ManagerLoginCss() {
    ...
    try {
      ...
      entry = await withPdfFetchRetry(async () => {
        const [studentRes, msRes] = await Promise.all([
          fetchRemoteStudentPdf(api, `/external-grading/submissions/${submissionId}/pdfs/submission`).then(data => ({ data })),
          api.get(`/external-grading/submissions/${submissionId}/pdfs/markScheme`, {
            responseType: "blob",
            timeout: 120000,
          }),
        ]);
  ...
  const getStudentFile = useCallback(
    async (submissionId) => fetchRemoteStudentPdf(api, `/external-grading/submissions/${submissionId}/pdfs/submission`).catch(async () => (await fetchPdfs(submissionId)).studentFile),
    [fetchPdfs]
  );
    ...
    if (automaticBatch) {
      try {
        const { data } = await api.post("/automatic-batch-queue", {
          ...
          classroomName: selectedAssignment.classroomName || selectedAssignment.classroom || null,
          ...
          config: { markingMode: mode, guidance: guidanceForForm(guidanceText), geminiModel: selectedModel, chunkSize: chunkSizeForGeminiModel(selectedModel), totalGrade: configuredTotal, ...examBoardGuidance.getExamBoardFields() },
        });
  ...
  const viewFeedback = async (student) => {
    try {
      const res = await api.get(`/external-grading/submissions/${student.submissionId}/feedback/file`, {
        responseType: "blob",
        timeout: 120000,
      });
                ...
                <div className="ma-panel-header">
                  ...
                  <div className="msv-panel-controls" style={{ flexWrap: "wrap", gap: 8 }}>
                    ...
                    <button
                      ...
                      disabled={selectedAssignment.id == null || bulkMarking || priorityBulkRunning || ["uploading", "submitting", "processing"].includes(batchJob?.phase)}
